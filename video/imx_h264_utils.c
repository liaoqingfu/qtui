...
{
	size_t const alloc_step_size = (4*length);
	size_t const read_size = (length);
	int num_aud_found = 0;
	size_t cur_offset = 0;
	uint8_t *newptr;
	...
	memcpy( ctx->in_buffer + ctx->in_buffer_data_size ,  buf , read_size);
	ctx->in_buffer_data_size += read_size;
...
/*
		...
		{
			ctx->au_end_offset = cur_offset;
			fprintf(stderr, "H264 DECODE Reading failed: %s", strerror(errno));
			return 0;
		}
