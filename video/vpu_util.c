...
#define VPU_FW_PATH_DEFAULT		"/usr/firmware/vpu"
...
RetCode LoadBitCodeTable(Uint16 * pBitCode, int *size)
{
...
#else
		strcpy(fw_name, VPU_FW_PATH_DEFAULT);	/* default path */
#endif
	else
	...
	if (fp == NULL) {
		...
		err_msg("Please put bin file to %s folder or export VPU_FW_PATH env\n",VPU_FW_PATH_DEFAULT);
